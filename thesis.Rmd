...
brig_temp <- cbrmrt1 %>% pivot_longer(
    cols=!CHROM & !POS & !REF & !ALT,
    names_to = "isotype",
    values_to = "altref"
)
brig_temp <- brig_temp %>% inner_join(cb_len_isotype) %>% filter(altref==-1) %>% dplyr::select(POS,length,isotype) 
cbrmrt1_plot <- brig_temp %>% ggplot(aes(x=POS,y=length)) + geom_boxplot(alpha=0.2,aes(group=POS)) + geom_jitter(color="black", size=0.2, alpha=0.2) + xlim(15329043,15332022)
cbrmrt1_plot
```
